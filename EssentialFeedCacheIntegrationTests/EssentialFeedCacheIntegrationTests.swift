...
//
//  Created by Temur on 06/12/2023.
//
...
final class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func test_save_overridesItemsSavedOnASeparateInstance() {
        ...
        let saveExpOne = expectation(description: "Wait for save completion")
        sutToPerformFirstSave.save(firstFeed) { saveError in
            XCTAssertNil(saveError, "Expected to save feed successfully")
            saveExpOne.fulfill()
        }
        wait(for: [saveExpOne], timeout: 1.0)
        
        let saveExpTwo = expectation(description: "Wait for save completion")
        sutToPerformLastSave.save(lastFeed) { saveError in
            XCTAssertNil(saveError, "Expected to save feed successfully")
            saveExpTwo.fulfill()
        }
        wait(for: [saveExpTwo], timeout: 1.0)
    ...
    func makeSUT(file: StaticString = #file, line: UInt = #line) -> LocalFeedLoader {
        ...
        let store = try! CoreDataFeedStore.init(storeURL: storeURL, bundle: storeBundle)
    ...
    private func expect(sut: LocalFeedLoader, toLoad expectedFeed: [FeedImage], file: StaticString = #file, line: UInt = #line) {
        ...
        sut.load { result in
            ...
            case let .failure(error):
                XCTFail("Expected to complete successfully, got \(error) instead", file: file, line: line)
            }
            exp.fulfill()
        }
